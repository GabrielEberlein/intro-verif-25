...
let test (#a:Type) () : Exn a (requires True) (ensures fun r -> True) =
  raise DivZero;
  let x = divide 1 0 in
  assert False

let test2 (#a:Type) (x:int) : Exn int (requires True) (ensures fun r -> True) =
  if x = 0 then
    raise DivZero;
  assert (x <> 0);
  let y = 1 / x in
...
: Exn int (requires True)
          (ensures fun (r : result int) -> if y <> 0 then r == V (x/y) else r == E DivZero) // True)
= if y = 0 then raise DivZero else x/y
...
(* Una caché para gcd *) 
let cache_elem_t = e:(nat & nat & int){let (a,b,c)=e in c == gcd a b}
...
let find_in_cache (x y : nat) : ML (option (r:int{r == gcd x y})) =
  let rec aux (xs : list cache_elem_t) : ML (option (r:int{r == gcd x y})) =
    match xs with
    | [] -> None
    | (a,b,c)::xs' ->  if x=a && y=b then Some c else aux xs'
  in
